...
// bianpang: 1.一个数据结构，想要持久化，就需要在磁盘上存储，但是针对该数据结构的计算，需要在内存中完成。
// bianpang: 2.数据结构的存储：一个数据结构就是一个完整的文件，每个节点node都在该文件中。
// bianpang: 3.对数据结构进行计算，比如初始化，插入，删除等操作的时候，不需要在内存上完全重建该数据结构，只需要逐个加载node，对node计算即可。所以内存上需要实现的是一个数据结构的入口，以及node，并实现对node的内存操作，以及磁盘和内存之间的转换操作。
type Tree struct {
	rootOff OFFTYPE
	nodePool *sync.Pool
	freeBlocks []OFFTYPE
